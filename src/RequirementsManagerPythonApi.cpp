...
NB_MODULE(FRRequirements, m) {
  ...
  nanobind::class_<ThreadPool<WorkerThread>>(m, "ThreadPool")
      .def(nanobind::new_(
          []() { return std::make_shared<ThreadPool<WorkerThread>>(); }))
      .def("status", &ThreadPool<WorkerThread>::status,
           "Returns ThreadState status of the thread pool")
      .def("startThreads", &ThreadPool<WorkerThread>::startThreads,
           "Start threads. Int parameter is the number of threads to start. "
           "Between 2 and 4 are usually good numbers.")
      .def("workerStatus", &ThreadPool<WorkerThread>::workerStatus,
           "Returns an array with the ThreadState status of each worker.")
      .def("hasWork", &ThreadPool<WorkerThread>::hasWork,
           "Returns true if work is currently queued up in the threadpool")
      .def("enqueue", &ThreadPool<WorkerThread>::enqueue,
           "Queue up a worker to be run whenever the threadpool gets around to "
           "it.")
      .def("shutdown", &ThreadPool<WorkerThread>::shutdown,
           "Shut down the threadpool. Threadpool will process remaining work "
           "before finally shutting down.")
      .def("join", &ThreadPool<WorkerThread>::join,
  ...
  nanobind::class_<SaveNodesNode<WorkerThread>, TaskNode<WorkerThread>>(m, "SaveNodesNode")
      ...
      .def("saveComplete", &SaveNodesNode<WorkerThread>::saveComplete,
           "Returns true when your starting node has been saved")
      .def("treeSaveComplete", &SaveNodesNode<WorkerThread>::treeSaveComplete,
           "Returns true when the entire graph associated with your node has "
           "been saved.");
  ...
  nanobind::class_<PqNodeFactory<WorkerThread>, TaskNode<WorkerThread>>(m, "PqNodeFactory")
      ...
      .def("getNode", &PqNodeFactory<WorkerThread>::getNode,
           "Retrieves the node you asked to be loaded.")
      .def("graphLoaded", &PqNodeFactory<WorkerThread>::graphLoaded,
           "Returns true if your graph is loaded. The ThreadPool could still "
           "be busy loading node data into the nodes. You can check its "
           "workerStatus to see if it's still doing anything.");
  ...
  nanobind::class_<Node>(m, "Node")
      ...
      .def_rw("up", &Node::up,
              "Node up-list. This indicates some sort of owner/parent "
              "relationship.")
      .def_rw("down", &Node::down,
              "Node down-list. This indicates some sort of owned/child "
              "relationship")
      .def_rw("changed", &Node::changed,
              "Indicates some data in the node changed.")
      .def("idString", &Node::idString,
           "Unique (UUIDV7) ID. You must call init to set the id, initally.")
      ...
      .def("to_json", &Node::to_json,
           "Returns JSON for this node. Includes entire graph accessible by "
           "node up and down lists.");
  ...
  nanobind::class_<CommitableNode, Node>(m, "CommitableNode")
      ...
      .def("commit", &CommitableNode::commit,
           "Commits this node, making further change impossible. This is for "
           "traceability -- to make a change to a committed node, create a "
           "pointer of this node type and call addChangeChild to the node you "
           "want to add the change to.")
      .def("isCommitted", &CommitableNode::isCommitted,
           "Checks committed status of this node.")
      .def("getChangeChild", &CommitableNode::getChangeChild,
           "Traverses the change child nodes. You can call getChangeChild "
           "recursively on change children until you receieve a None back. "
           "None indicates there are no more change children.")
      .def("getChangeParent", &CommitableNode::getChangeParent,
           "Traverses the change parent nodes. It will return None if you're "
           "at the top level parent.")
      .def("addChangeChild", &CommitableNode::addChangeChild,
           "Add a change child at the end of the changeChild tree.")
      .def("discardChange", &CommitableNode::discardChange,
           "Attempt to discard the changeChild of the current node. The object "
           "will refuse to do so if the current changeChild is already "
           "committed.");
  ...
  nanobind::class_<Organization, Node>(m, "Organization")
      ...
      .def("isLocked", &Organization::isLocked,
           "Checks to see if the organization is locked. If it's locked you "
           "can't change its name.")
      .def("setName", &Organization::setName,
           "Sets the name of the organization")
      .def("getName", &Organization::getName,
           "Gets the name of the organization")
      .def("lock", &Organization::lock,
           "Locks the organization, preventing its name from being changed.")
      .def("unlock", &Organization::unlock,
           "Unlocks the organization, allowing its name to be changed.");
  ...
  nanobind::class_<Product, CommitableNode>(m, "Product")
      ...
      .def("setDescription", &Product::setDescription,
           "Set the product description")
      .def("getTitle", &Product::getTitle, "Get the product name")
      .def("getDescription", &Product::getDescription,
  ...
  nanobind::class_<Project, Node>(m, "Project")
      ...
      .def("setDescription", &Project::setDescription,
           "Sets the project description")
      .def("getName", &Project::getName, "Gets the project name")
      .def("getDescription", &Project::getDescription,
  ...
  nanobind::class_<Requirement, CommitableNode>(m, "Requirement")
      ...
      .def("setFunctional", &Requirement::setFunctional,
           "Set whether the requirement is functional or non-functional. Pass "
           "it a True/False")
      .def("getTitle", &Requirement::getTitle, "Get the requirement title")
      .def("getText", &Requirement::getText, "Get the requirement text")
      .def("isFunctional", &Requirement::isFunctional,
           "Returns true if the requirement is a functional requirement");
  ...
  nanobind::class_<Completed, Node>(m, "Completed")
      ...
      .def("setDescription", &Completed::setDescription,
           "Set the description for this completed node.")
      .def("getDescription", &Completed::getDescription,
           "Get the description for this completed node.");
  ...
  nanobind::class_<TimeEstimate, Node>(m, "TimeEstimate")
      ...
      .def("setEstimate", &TimeEstimate::setEstimate,
           "Set the estimate (Duration, seconds) for this node")
      .def("getEstimate", &TimeEstimate::getEstimate,
           "Gets the estimate (Duration, seconds) for this node")
      .def("setStarted", &TimeEstimate::setStarted,
           "Indicates work on this estimate has started.")
      .def("getStarted", &TimeEstimate::getStarted,
           "Indicates work on this estimate has started.")
      .def("setStartTimestamp", &TimeEstimate::setStartTimestamp,
           "Date work on this estimate started.")
      .def("getStartTimestamp", &TimeEstimate::getStartTimestamp,
           "Date work on this estimate started.");
  ...
  nanobind::class_<Effort, Node>(m, "Effort")
      ...
      .def("setEffort", &Effort::setEffort,
           "Set the effort (time spent) (Duration, seconds) for this node. You "
           "can drop more than one effort node in the down list of a node "
           "where time-worked on the thing makes sense.")
      .def("getEffort", &Effort::getEffort,
           "Get the effort (duration, seconds) for this node");
  ...
  nanobind::class_<Goal, Node>(m, "Goal")
      ...
      .def("setAction", &Goal::setAction,
           "Set the action (what will be done) part of this goal")
      .def("setOutcome", &Goal::setOutcome,
           "Set the outcome (What defines success) part of this goal")
      .def(
          "setContext", &Goal::setContext,
          "Set the context (who/what is this goal targeting) part of this goal")
      .def("setTargetDate", &Goal::setTargetDate,
           "Set the target date/Deadline of the goal (Python Datetime object)")
      .def("setTargetDateConfidence", &Goal::setTargetDateConfidence,
           "Set target date confidence/priority/flexibiltiy (string tag)")
      .def("setAlignment", &Goal::setAlignment,
           "Set alignment (Why we have goal first place.)")
      ...
      .def("getTargetDate", &Goal::getTargetDate,
           "Get the target date/deadline of this goal")
      .def("getTargetDateConfidence", &Goal::getTargetDateConfidence,
           "Get the target date confidence/flexibility/priority (string tag)");

  nanobind::class_<Purpose, Node>(m, "Purpose")
      .def(nanobind::new_([]() { return std::make_shared<Purpose>(); }))
      .def("setDescription", &Purpose::setDescription,
           "Set the description of this purpose (IE: You pass butter)")
      .def("setDeadline", &Purpose::setDeadline,
           "Set the deadline for this purpose (Python datetime object)")
      .def("setDeadlineConfidence", &Purpose::setDeadlineConfidence,
           "Set the confidence/wiggle room/priority of this deadline (string "
           "tag)")
      .def("getDescription", &Purpose::getDescription,
           "Get the description of this Purpose")
      .def("getDeadline", &Purpose::getDeadline,
           "Get the deadline for this Purpose (Python datetime object)")
      .def("getDeadlineConfidence", &Purpose::getDeadlineConfidence,
           "Get the confdience/wiggle room/priority of this Purpose (String "
           "tag)");
  ...
  nanobind::class_<PhoneNumber, Node>(m, "PhoneNumber")
      ...
      .def("setCountryCode", &PhoneNumber::setCountryCode,
           "Set country code (nominally optional if you're a one country "
           "project.)")
      .def("setNumber", &PhoneNumber::setNumber, "Set phone number")
      .def("setPhoneType", &PhoneNumber::setPhoneType,
           "Set phone type (home, office etc)")
      .def("getCountryCode", &PhoneNumber::getCountryCode, "Get country code.")
      .def("getNumber", &PhoneNumber::getNumber, "Get phone number")
      .def("getPhoneType", &PhoneNumber::getPhoneType,
           "Get phone type (home, office etc)");

  nanobind::class_<InternationalAddress, Node>(m, "InternationalAddress")
      .def(nanobind::new_(
          []() { return std::make_shared<InternationalAddress>(); }))
      .def("setCountryCode", &InternationalAddress::setCountryCode,
           "Set country code (ISO 3166-1 Country codes)")
      .def("setAddressLines", &InternationalAddress::setAddressLines,
           "Set an address line Text node. If you need additional address "
           "lines, put them in the down list of the text node you set this to.")
      .def("setLocality", &InternationalAddress::setLocality,
           "Set Locality (city/town)")
      .def("setPostalCode", &InternationalAddress::setPostalCode,
           "set Postal/Zip Code")
      .def("getCountryCode", &InternationalAddress::getCountryCode,
           "Get country code")
      .def("getAddressLines", &InternationalAddress::getAddressLines,
           "Get address lines text node.")
      .def("getLocality", &InternationalAddress::getLocality,
           "Get locality (City/Town)")
      .def("getPostalCode", &InternationalAddress::getPostalCode,
           "Get Postal/Zip code");
  ...
  nanobind::class_<Event, Node>(m, "Event")
      ...
      .def("getDescription", &Event::getDescription,
           "Gets the description of this event")
      .def("setDescription", &Event::setDescription,
